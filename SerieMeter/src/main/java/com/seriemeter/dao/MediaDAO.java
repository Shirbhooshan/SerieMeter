...
public class MediaDAO {
    ...
    public boolean insertMedia(MediaModel media) {
        ...
        String sql = "INSERT INTO media (title, director, release_date, description, media_profile, category_id, genre_id) "
                   + "VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        try (Connection con = DBconfig.getConnection();
            ...
            pst.setDate(3, java.sql.Date.valueOf(media.getReleaseDate())); // "YYYY-MM-DD" string → SQL Date
            pst.setString(4, media.getDescription());
            pst.setString(5, media.getMediaProfile());
    ...
    public MediaModel getMediaById(int mediaId) {
        ...
        try (Connection con = DBconfig.getConnection();
            ...
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) { // if() not while() — only one row expected
    ...
    public boolean updateMedia(MediaModel media) {
    	...
    	String sql = "UPDATE media SET title = ?, director = ?, release_date = ?, "
    	           + "description = ?, category_id = ?, genre_id = ?, media_profile = ? "
    	           + "WHERE media_id = ?";
